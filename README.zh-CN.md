...
2. **提案 (proposing)** — 确定范围、非目标、成功标准
3. **规范 (specifying)** — 用 GIVEN/WHEN/THEN 定义可测试的行为，这是整个工作流的脊柱
4. **设计 (designing)** — 技术架构，带有取舍记录
5. **规划 (planning)** — 细粒度 TDD 任务，每个都映射到规范场景
6. **执行 (executing)** — 严格 RED→GREEN→REFACTOR，每个任务完成后自动代码审查
7. **归档 (archiving)** — 将增量规范合并到主规范

每一行代码都能追溯到一份规范。没有规范就不写代码。
...
**你掌控 git。** AI 永远不会执行 git 命令。每个任务完成后暂停，由你 review 和 commit。

**行为塑造。** 每个技能都有红旗预警表、铁律和借口防御——来自真实失败模式的硬约束，不是建议。

**角色隔离。** AI 在每个阶段扮演不同的受限角色：
...
| 设计 | 系统架构师 | 写代码 |
| 规划 | Tech Lead | 开始实现 |
| 执行 | 开发工程师 | 跳过 TDD 或改规范 |

**双执行模式。** 逐任务（默认）：一个任务 → 审查 → commit → 继续。快速模式：全部任务 → 统一审查 → 一次性 commit。
