...
Codex 插件安装使用 `codex plugin marketplace`。仓库里的 `.agents/plugins/marketplace.json` 暴露插件，`.codex-plugin/plugin.json` 指向源码中的 `skills/` 目录，因此不生成 `.codex/skills/`。

生成的 Claude Code 插件技能产物和 `manifests/install-state/` 状态文件属于本地安装产物，不是源码。

### 语言规则

Claude Code 插件技能产物在安装阶段生成。`developer` 配置默认包含 `rules-common`；语言特定规则需要在生成受管产物时显式加入。Codex 直接从插件 manifest 的 `skills` 路径读取源码规则。
